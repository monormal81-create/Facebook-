...
# CREATE POST
# =========================
from app.posts.hashtag_service import extract_hashtags
from app.posts.hashtag_service import format_hashtags
def create_post(
    db: Session,
    user: User,
    content: str,
    hashtags_list = extract_hashtags(content)
hashtags_str = format_hashtags(hashtags_list)
    image: str = None
):
...
def delete_post(
    ...
    if post.user_id != user.id:
        raise ValueError("You cannot delete this post")
